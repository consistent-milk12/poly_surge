...
use algebra::{DMat3, DVec3, DVec4};
use itertools::Itertools;

use crate::incremental_hull::IncrementalHull;

// Tracing macros - compile to no-ops without the "tracing" feature
#[cfg(feature = "tracing")]
macro_rules! trace_clip {
            ...
            for dy in -1..=1 {
                for dz in -1..=1 {
                    if let Some(pts) = self.cells.get(&(cx + dx, cy + dy, cz + dz))
                        && pts.iter().any(|&p| (p - point).norm() < self.tolerance)
                    {
                        return true;
                    }
                }
...
impl HalfSpace {
    ...
    pub fn new_normalized(normal: DVec3, offset: f64) -> Self {
        debug_assert!((normal.norm() - 1.0).abs() < EPSILON * 100.0);
        Self { normal, offset }
    }
...
impl Edge {
    ...
    #[must_use]
    pub fn canonical_planes(p1: PlaneIdx, p2: PlaneIdx) -> (PlaneIdx, PlaneIdx) {
...
pub enum TopologyError {
    ...
    UnderconstrainedVertex { vertex: VertexIdx, plane_count: usize },
    /// Ideal vertex has fewer than 2 incident planes.
    InvalidIdealVertex { vertex: VertexIdx, plane_count: usize },
    /// Edge references a non-existent vertex.
    DanglingEdge { edge: EdgeIdx, missing_vertex: VertexIdx },
    /// Face (plane) has fewer than 3 vertices.
    DegenerateFace { plane: PlaneIdx, vertex_count: usize },
...
impl std::fmt::Display for TopologyError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::EulerMismatch { vertices, edges, faces, expected, actual } => {
                write!(f, "Euler mismatch: V={vertices}, E={edges}, F={faces}, χ={actual} (expected {expected})")
            }
            Self::UnderconstrainedVertex { vertex, plane_count } => {
                write!(f, "Vertex {:?} has only {plane_count} incident planes (need ≥3)", vertex)
            }
            Self::InvalidIdealVertex { vertex, plane_count } => {
                write!(f, "Ideal vertex {:?} has only {plane_count} incident planes (need ≥2)", vertex)
            }
            Self::DanglingEdge { edge, missing_vertex } => {
                write!(f, "Edge {:?} references non-existent vertex {:?}", edge, missing_vertex)
            }
            Self::DegenerateFace { plane, vertex_count } => {
                write!(f, "Face {:?} has only {vertex_count} vertices (need ≥3)", plane)
            }
...
pub struct IncrementalPolytope {
    ...
    // Fast lookups
    edge_map: HashMap<(PlaneIdx, PlaneIdx), EdgeIdx>, // (plane1, plane2) → edge
    ...
    edges_dirty: bool,
    dirty_vertices: HashSet<VertexIdx>, // Which vertices need edge rebuild (O(1) insert/contains)
    faces_dirty: bool,
    dirty_faces: HashSet<PlaneIdx>, // Which faces need ordering rebuild (O(1) insert/contains)

    // Cached data
    face_orderings: HashMap<PlaneIdx, Vec<VertexIdx>>, // Sorted vertices per face
...
impl IncrementalPolytope {
    ...
    fn classify_vertices(&self, plane_idx: PlaneIdx) -> Vec<(VertexIdx, Classification)> {
        ...
        plane.map_or(vec![], |p| {
            self.vertices()
                .map(|(idx, v)| {
                    let classification = if let Some(pos) = v.to_euclidean() {
                        // Finite vertex: standard classification
                        p.classify(pos, self.epsilon)
                    } else {
    ...
    fn find_initial_simplex(&self, valid_indices: &[usize]) -> Option<[usize; 4]> {
        ...
        let det = DMat3::from_columns(&[selected[0].1, selected[1].1, selected[2].1]).determinant();
    ...
    fn clip_by_plane(
        ...
        trace_clip!("clip_by_plane: processing {} edges", edge_indices.len());
            ...
            match (c1, c2) {
                ...
                _ => {
                    ...
                    let new_vertex_pos = if v0.is_finite() && v1.is_finite() {
                        ...
                        trace_clip!("       segment: p1={:?}, p2={:?}", p1, p2);
                        ...
                        trace_clip!("       ray (v0 finite): origin={:?}, dir={:?}", p, d);
                        let result = compute_ray_plane_intersection(p, d, plane, self.epsilon);
                        trace_clip!("       ray result: {:?}", result);
                        ...
                        trace_clip!("       ray (v1 finite): origin={:?}, dir={:?}", p, d);
                        let result = compute_ray_plane_intersection(p, d, plane, self.epsilon);
                        trace_clip!("       ray result: {:?}", result);
                        result.map(|p| DVec4::new(p.x, p.y, p.z, 1.0))
                    } else {
                        // Line (both ideal): intersection is the 3-plane intersection
                        // The line is defined by edge_planes, intersect with the clipping plane
                        trace_clip!("       line (both ideal)");
                    ...
                    trace_clip!("       new_vertex_pos: {:?}", new_vertex_pos);
            ...
            for other_v in candidates {
                ...
                if shared.len() >= 2 {
                        ...
                        self.edge_by_planes_internal(edge_planes.0, edge_planes.1)
                    {
                        ...
                        if let Some(v) = self.vertex_mut(dirty_v) {
                            if !v.edges.contains(&existing_e_idx) {
    ...
    fn reconstruct_vertices_topology_based(
        ...
        // Use spatial hash for O(1) duplicate detection of finite vertices
        let mut spatial_hash = SpatialHash::new(self.epsilon * 10.0);

        // Track ideal vertices by their defining plane pair and direction sign
        // Key: (min_plane, max_plane, positive_direction: bool)
        let mut ideal_vertices_seen: HashSet<(PlaneIdx, PlaneIdx, bool)> = HashSet::new();
        ...
        for (_, v) in self.vertices() {
            if !v.is_finite() && v.planes.len() >= 2 {
                ...
                // Determine direction sign based on first component of direction
                let dir = dvec4_xyz(&v.position);
                let positive =
                    dir.x > 0.0 || (dir.x == 0.0 && (dir.y > 0.0 || (dir.y == 0.0 && dir.z > 0.0)));

                ideal_vertices_seen.insert((min_p, max_p, positive));
            }
        }
        ...
        for (_v_idx, position_hom, remaining_planes) in removed_vertex_data {
            ...
            for (i, &p1_idx) in remaining_planes.iter().enumerate() {
                ...
                for &p2_idx in remaining_planes.iter().skip(i + 1) {
                    ...
                    if line_dir.norm_squared() < self.epsilon * self.epsilon {
                    ...
                    for sign in [-1.0, 1.0] {
                            ...
                            self.line_search_next_plane(position, dir, p1_idx, p2_idx)
                        {
                            ...
                            if !spatial_hash.insert_if_unique(new_point) {
                                continue;
                            }

                            // Collect all incident planes (for degeneracy handling)
                            let mut incident_planes = vec![p1_idx.0, p2_idx.0, hit_plane_idx.0];
...
pub fn intersect_three_planes(p1: &HalfSpace, p2: &HalfSpace, p3: &HalfSpace) -> Option<DVec4> {
    ...
    let coeffs = DMat3::from_columns(&[p1.normal, p2.normal, p3.normal]).transpose();

    coeffs
        .lu()
        .solve(&constraints)
        .map(|pos| DVec4::new(pos.x, pos.y, pos.z, 1.0))
}
...
mod tests {
    ...
    /// Initialize tracing for tests (only with the "tracing" feature).
    /// Call this at the start of any test that needs detailed trace output.
    /// Run with: `cargo test --release --features tracing test_name -- --nocapture`
    #[cfg(feature = "tracing")]
    #[allow(dead_code)]
    fn init_tracing() {
        ...
        let _ = fmt()
            .with_env_filter(
                EnvFilter::from_default_env().add_directive("geometry=trace".parse().unwrap()),
            )
            .without_time()
            .with_target(false)
            .with_level(false)
    ...
    fn test_three_plane_intersection() {
        ...
        let point = dvec4_xyz(&point_hom); // Extract Euclidean position
        assert!((point - DVec3::zeros()).norm() < EPSILON);
    ...
    #[test]
    fn test_compare_with_vertex_enumeration_cube() {
        use algebra::na::DMatrix;

        use crate::vertex_enumeration_from_half_spaces;

        // Use cube planes for comparison
        let directions = vec![
            DVec3::new(1.0, 0.0, 0.0),
            DVec3::new(-1.0, 0.0, 0.0),
            DVec3::new(0.0, 1.0, 0.0),
        ...
        // Build using incremental polytope
        let mut poly = IncrementalPolytope::new();
        for dir in &directions {
            poly.add_plane(HalfSpace::new(*dir, 0.5));
        }

        // Build using existing vertex enumeration
        let mut planes_matrix = DMatrix::from_element(directions.len(), 4, 0.0);
        for (i, dir) in directions.iter().enumerate() {
            planes_matrix[(i, 0)] = dir.x;
            planes_matrix[(i, 1)] = dir.y;
            planes_matrix[(i, 2)] = dir.z;
            planes_matrix[(i, 3)] = 0.5;
        }

        let (ref_vertices, _ref_faces) = vertex_enumeration_from_half_spaces(&planes_matrix);

        let (inc_vertices, _inc_faces) = poly.to_mesh();

        // Compare vertex counts
        assert_eq!(
            inc_vertices.len(),
            ref_vertices.len(),
            "Vertex count mismatch: incremental={}, reference={}",
            inc_vertices.len(),
            ref_vertices.len()
        );

        // Verify all incremental vertices exist in reference (within tolerance)
        for v in &inc_vertices {
            let found = ref_vertices.iter().any(|rv| (*v - *rv).norm() < 1e-6);
            assert!(found, "Vertex {v:?} not found in reference");
        }
    }

    #[test]
    fn test_compare_with_vertex_enumeration_octahedron() {
        use algebra::na::DMatrix;

        use crate::vertex_enumeration_from_half_spaces;

        // Use octahedron planes (8 planes with diagonal normals)
        ...
        let mut poly = IncrementalPolytope::new();
        for dir in &directions {
            poly.add_plane(HalfSpace::new(*dir, 1.0));
        }
    ...
    fn test_three_plane_corner_case() {
        ...
        assert!(pos.norm() < EPSILON, "Finite vertex should be at origin");
    ...
    fn test_icosahedron_construction() {
        ...
        let mut mat = DMatrix::from_vec(0, 4, vec![]);
        for normal in &normals {
            let n = normal.normalize();
            let d = 2.0;
            let row = mat.nrows();
            mat = mat.insert_row(row, 0.0);
            mat.set_row(row, &crate::na::RowVector4::new(n.x, n.y, n.z, d));
        }
        let (old_verts, old_faces) = vertex_enumeration_from_half_spaces(&mat);
        ...
        // Euler: 12 - 30 + 20 = 2 ✓
        //
        // Verified against old vertex_enumeration_from_half_spaces implementation.
        // The original expectation of 36/54/20 was incorrect.
        assert_eq!(poly.vertex_count(), old_verts.len());
        assert_eq!(poly.vertex_count(), 12);
        assert_eq!(poly.edge_count(), 30);

        let (vertices, faces) = poly.to_mesh();
        assert_eq!(vertices.len(), old_verts.len());
        assert_eq!(faces.len(), old_faces.len());
        assert_eq!(vertices.len(), 12);
        assert_eq!(faces.len(), 20);
    }

    /// Port of test_vertex_enumeration_from_half_spaces_edge_case from vertex_enumeration.rs
    ...
    fn test_edge_case_polytope() {
        ...
        for (nx, ny, nz, d) in planes_data {
            let row = mat.nrows();
            mat = mat.insert_row(row, 0.0);
            mat.set_row(row, &crate::na::RowVector4::new(nx, ny, nz, d));
        }
        let (old_verts, old_faces) = vertex_enumeration_from_half_spaces(&mat);
        ...
        assert_eq!(
            vertices.len(),
            old_verts.len(),
            "Vertex count should match old implementation"
        );
        assert_eq!(
            faces.len(),
            old_faces.len(),
            "Face count should match old implementation"
        );
    ...
    fn test_validate_topology_cube() {
        ...
        assert!(poly.validate_topology().is_ok(), "Cube should have valid topology");
    }
    ...
    fn test_validate_topology_after_removal() {
        ...
        for i in 0..20 {
            ...
            let phi = (1.0 - 2.0 * (i as f64 + 0.5) / 20.0).acos();
        ...
        assert!(poly.validate_topology().is_ok(), "Should be valid before removal");

        // Remove a plane
        poly.remove_plane(PlaneIdx(5));

        // Should still be valid after removal (bug was fixed)
        assert!(poly.validate_topology().is_ok(), "Should be valid after removal");
    }
}
