...
impl IncrementalHull {
    ...
    #[must_use]
    pub const fn origin_inside(&self) -> bool {
        self.origin_inside
    }
    ...
    #[inline]
    const fn is_valid_point(point: &DVec3) -> bool {
    ...
    fn compute_origin_inside(&self) -> bool {
        ...
        for (face_idx, face) in self.active_faces() {
            ...
            if dist >= -self.epsilon * normal.length() {
                return false;
            }
        }
...
mod convex_hull_f64 {
    ...
    /// Compute the 2D convex hull of coplanar 3D points using gift wrapping.
    ///
    /// Returns indices into the original point set (0..n), ordered counter-clockwise.
    /// The `get_point` function maps an index to a 3D point on the plane.
    ///
    /// # Algorithm
    /// 1. Find the normal of the plane using the first 3 non-collinear points
    /// 2. Project all points onto a 2D coordinate system on that plane
    /// 3. Perform gift wrapping (Jarvis march) in 2D
    /// 4. Return the indices in hull order
    fn indexed_planar_convex_hull<F>(get_point: &F, n: usize) -> Vec<usize>
    where
        F: Fn(usize) -> DVec3,
    {
        if n < 3 {
        ...
        // Find a plane normal using first 3 points
        let p0 = get_point(0);
        let p1 = get_point(1);
        let p2 = get_point(2);
        ...
        if normal_len < COPLANAR_TOL {
            // Points are collinear - return them all in order
            return (0..n).collect();
        }
        ...
        let points_2d: Vec<(f64, f64)> = (0..n)
            .map(|i| {
                let p = get_point(i) - p0;
        ...
        let start = (0..n)
            .min_by(|&a, &b| {
                points_2d[a]
                    .0
                    .partial_cmp(&points_2d[b].0)
                    .unwrap_or(std::cmp::Ordering::Equal)
                    .then_with(|| {
                        points_2d[a]
                            .1
                            .partial_cmp(&points_2d[b].1)
                            .unwrap_or(std::cmp::Ordering::Equal)
                    })
            })
            .unwrap_or(0);
        ...
        loop {
            ...
            let mut next = usize::from(current == 0);
            ...
            for candidate in 0..n {
                ...
                let cross = cross_2d(&points_2d[current], &points_2d[next], &points_2d[candidate]);

                // If candidate is more counter-clockwise (or collinear but farther)
                if cross > COPLANAR_TOL
                    || (cross.abs() <= COPLANAR_TOL
                        && dist_sq_2d(&points_2d[current], &points_2d[candidate])
                            > dist_sq_2d(&points_2d[current], &points_2d[next]))
                {
                    next = candidate;
                }
            }
    ...
    /// 2D cross product: (b - a) × (c - a). Positive = c is counter-clockwise from b.
    ...
    fn has_non_colinear(points: &[usize], coords: &[DVec3]) -> bool {
        ...
        for i in 1..points.len() {
            ...
            for j in i + 1..points.len() {
                ...
                if (p1 - p0).cross(p2 - p0).length() >= COPLANAR_TOL {
                    return true;
                }
            }
        }
    ...
    fn triangulate_coplanar_points(
        ...
        for &idx in coplanar_points.iter() {
            if deduped
                .iter()
                .all(|&u| (points[u] - points[idx]).length() >= COPLANAR_TOL)
            {
                deduped.push(idx);
            }
        }
...
mod tests {
        ...
        for (face_idx, adj_opt) in hull.face_adjacency.iter().enumerate() {
            ...
            for (edge_idx, &adj_face) in adj.iter().enumerate() {
                if adj_face != usize::MAX {
                    assert!(
                        hull.faces.get(adj_face).is_some_and(Option::is_some),
    ...
    fn test_icosahedron_normals_hull() {
        ...
        let hull = IncrementalHull::build(
            normals
                .iter()
                .enumerate()
        ...
        eprintln!(
            "Hull point indices on hull: {}",
            hull.hull_point_indices.len()
        );
        ...
        assert!(
            hull.origin_inside(),
            "Origin should be inside hull of 20 normals spanning all directions"
        );
    }
}
